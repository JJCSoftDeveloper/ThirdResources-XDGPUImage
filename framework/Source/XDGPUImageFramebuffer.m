...
@interface  XDGPUImageFramebuffer()
{
    GLuint framebuffer;
...
@implementation  XDGPUImageFramebuffer
...
- (void)generateFramebuffer;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            CVOpenGLESTextureCacheRef coreVideoTextureCache = [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache];
...
- (void)destroyFramebuffer;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([XDGPUImageContext supportsFastTextureUpload] && (!_missingFramebuffer))
        {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            if (renderTarget)
            {
...
- (void)unlock;
{
    ...
    {
        [[XDGPUImageContext sharedFramebufferCache] returnFramebufferToCache:self];
    }
}
...
void dataProviderUnlockCallback (void *info, const void *data, size_t size)
{
     XDGPUImageFramebuffer *framebuffer = (__bridge_transfer  XDGPUImageFramebuffer*)info;
    
    [framebuffer restoreRenderTarget];
    [framebuffer unlock];
    [[XDGPUImageContext sharedFramebufferCache] removeFramebufferFromActiveImageCaptureList:framebuffer];
}
...
- (CGImageRef)newCGImageFromFramebufferContents;
{
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([XDGPUImageContext supportsFastTextureUpload])
        {
            ...
            [[XDGPUImageContext sharedFramebufferCache] addFramebufferToActiveImageCaptureList:self]; // In case the framebuffer is swapped out on the filter, need to have a strong reference to it somewhere for it to hang on while the image is in existence
#else
#endif
        }
        else
...
- (void)unlockAfterReading
{
    ...
    if ([XDGPUImageContext supportsFastTextureUpload])
    {
        NSAssert(readLockCount > 0, @"Unbalanced call to -[XDGPUImageFramebuffer unlockAfterReading]");
        readLockCount--;
        if (readLockCount == 0)
        {
