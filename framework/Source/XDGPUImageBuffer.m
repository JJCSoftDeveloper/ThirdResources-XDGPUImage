...
@interface  XDGPUImageBuffer()

@end

@implementation  XDGPUImageBuffer
...
- (void)dealloc
{
    for (XDGPUImageFramebuffer *currentFramebuffer in bufferedFramebuffers)
    {
...
- (void)setBufferSize:(NSUInteger)newValue;
{
    ...
    else
    {
        ...
        {
             XDGPUImageFramebuffer *lastFramebuffer = [bufferedFramebuffers lastObject];
