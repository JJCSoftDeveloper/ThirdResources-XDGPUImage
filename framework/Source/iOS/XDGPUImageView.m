...
@interface XDGPUImageView () 
{
    XDGPUImageFramebuffer *inputFramebufferForDisplay;
...
- (void)commonInit;
{
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        displayProgram = [[XDGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:kGPUImagePassthroughFragmentShaderString];
        ...
        [XDGPUImageContext setActiveShaderProgram:displayProgram];
...
- (void)createDisplayFramebuffer;
{
    ...
    [[[XDGPUImageContext sharedImageProcessingContext] context] renderbufferStorage:GL_RENDERBUFFER fromDrawable:(CAEAGLLayer*)self.layer];
...
{
    glBindRenderbuffer(GL_RENDERBUFFER, displayRenderbuffer);
    [[XDGPUImageContext sharedImageProcessingContext] presentBufferForDisplay];
}
...
+ (const GLfloat *)textureCoordinatesForRotation:(XDGPUImageRotationMode)rotationMode;
{
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        glVertexAttribPointer(displayTextureCoordinateAttribute, 2, GL_FLOAT, 0, 0, [XDGPUImageView textureCoordinatesForRotation:inputRotation]);
...
- (void)setInputSize:(CGSize)newSize atIndex:(NSInteger)textureIndex;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if (XDGPUImageRotationSwapsWidthAndHeight(inputRotation))
        {
...
- (void)setFillMode:(XDGPUImageFillModeType)newValue;
{
    _fillMode = newValue;
    [self recalculateViewGeometry];
}
