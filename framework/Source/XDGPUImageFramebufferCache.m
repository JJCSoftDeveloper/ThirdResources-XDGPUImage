...
@interface  XDGPUImageFramebufferCache()
{
...
@implementation  XDGPUImageFramebufferCache
...
{
    __block  XDGPUImageFramebuffer *framebufferFromCache = nil;
//    dispatch_sync(framebufferCacheQueue, ^{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([numberOfMatchingTexturesInCache integerValue] < 1)
        {
            ...
            framebufferFromCache = [[XDGPUImageFramebuffer alloc] initWithSize:framebufferSize textureOptions:textureOptions onlyTexture:onlyTexture];
        }
        else
        {
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
        CVOpenGLESTextureCacheFlush([[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], 0);
#else
#endif
    });
}
