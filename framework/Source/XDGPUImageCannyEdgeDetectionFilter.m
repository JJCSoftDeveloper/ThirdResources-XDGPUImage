...
@implementation  XDGPUImageCannyEdgeDetectionFilter
...
- (id)init;
{
    ...
    luminanceFilter = [[XDGPUImageGrayscaleFilter alloc] init];
    [self addFilter:luminanceFilter];
    
    // Second pass: apply a variable Gaussian blur
    blurFilter = [[XDGPUImageSingleComponentGaussianBlurFilter alloc] init];
    [self addFilter:blurFilter];
    
    // Third pass: run the Sobel edge detection, with calculated gradient directions, on this blurred image
    edgeDetectionFilter = [[XDGPUImageDirectionalSobelEdgeDetectionFilter alloc] init];
    [self addFilter:edgeDetectionFilter];
    
    // Fourth pass: apply non-maximum suppression    
    nonMaximumSuppressionFilter = [[XDGPUImageDirectionalNonMaximumSuppressionFilter alloc] init];
    [self addFilter:nonMaximumSuppressionFilter];
    
    // Fifth pass: include weak pixels to complete edges
    weakPixelInclusionFilter = [[XDGPUImageWeakPixelInclusionFilter alloc] init];
