...
@implementation  XDGPUImageLowPassFilter
...
- (id)init;
{
    ...
    dissolveBlendFilter = [[XDGPUImageDissolveBlendFilter alloc] init];
    [self addFilter:dissolveBlendFilter];
    
    // Buffer the result to be fed back into the blend
    bufferFilter = [[XDGPUImageBuffer alloc] init];
...
- (void)addTarget:(id<XDGPUImageInput>)newTarget atTextureLocation:(NSInteger)textureLocation;
{
    ...
    //if use  XDGPUImagePipline,will cause self.termainlFilter removeAllTargets,so need add bufferFilter back
