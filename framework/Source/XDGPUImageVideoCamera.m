...
@interface XDGPUImageVideoCamera () 
{
...
@implementation XDGPUImageVideoCamera
...
- (id)initWithSessionPreset:(NSString *)sessionPreset cameraPosition:(AVCaptureDevicePosition)cameraPosition; 
{
	...
	// Add the video input	
	NSError *error = nil;
	self.videoInput = [[AVCaptureDeviceInput alloc] initWithDevice:_inputCamera error:&error];
...
//    if (captureAsYUV && [XDGPUImageContext deviceSupportsRedTextures])
    if (captureAsYUV && [XDGPUImageContext supportsFastTextureUpload])
    {
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if (captureAsYUV)
        {
            ...
            //            if ([XDGPUImageContext deviceSupportsRedTextures])
            //            {
            //                yuvConversionProgram = [[XDGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:kGPUImageYUVVideoRangeConversionForRGFragmentShaderString];
            //            }
            //            else
            //            {
            if (isFullYUVRange)
            {
                yuvConversionProgram = [[XDGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:kGPUImageYUVFullRangeConversionForLAFragmentShaderString];
            }
            else
            {
                yuvConversionProgram = [[XDGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:kGPUImageYUVVideoRangeConversionForLAFragmentShaderString];
            }
            ...
            [XDGPUImageContext setActiveShaderProgram:yuvConversionProgram];
    ...
    [self.videoOutput setSampleBufferDelegate:self queue:cameraProcessingQueue];
...
- (void)removeInputsAndOutputs;
{
    ...
    if (self.videoInput) {
        ...
        [_captureSession removeOutput:self.videoOutput];
        self.videoInput = nil;
...
- (BOOL)isBackFacingCameraPresent
{
    return [XDGPUImageVideoCamera isBackFacingCameraPresent];
}
...
- (BOOL)isFrontFacingCameraPresent
{
    return [XDGPUImageVideoCamera isFrontFacingCameraPresent];
}
...
- (void)processVideoSampleBuffer:(CMSampleBufferRef)sampleBuffer;
{
    ...
    if ([XDGPUImageContext supportsFastTextureUpload] && captureAsYUV)
    {
...
//                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, coreVideoTextureCache, cameraFrame, NULL, GL_TEXTURE_2D, GL_RED_EXT, bufferWidth, bufferHeight, GL_RED_EXT, GL_UNSIGNED_BYTE, 0, &luminanceTextureRef);
                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], cameraFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE, bufferWidth, bufferHeight, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0, &luminanceTextureRef);
            }
            else
            {
                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], cameraFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE, bufferWidth, bufferHeight, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0, &luminanceTextureRef);
...
//                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, coreVideoTextureCache, cameraFrame, NULL, GL_TEXTURE_2D, GL_RG_EXT, bufferWidth/2, bufferHeight/2, GL_RG_EXT, GL_UNSIGNED_BYTE, 1, &chrominanceTextureRef);
                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], cameraFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE_ALPHA, bufferWidth/2, bufferHeight/2, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 1, &chrominanceTextureRef);
            }
            else
            {
                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], cameraFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE_ALPHA, bufferWidth/2, bufferHeight/2, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 1, &chrominanceTextureRef);
...
//            }
            ...
            if (XDGPUImageRotationSwapsWidthAndHeight(internalRotation))
            {
...
//            CVPixelBufferLockBaseAddress(cameraFrame, 0);
//            
//            CVReturn err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], cameraFrame, NULL, GL_TEXTURE_2D, GL_RGBA, bufferWidth, bufferHeight, GL_BGRA, GL_UNSIGNED_BYTE, 0, &texture);
//            
//            if (!texture || err) {
...
//            outputTexture = 0;
    ...
    else
    {
        ...
        outputFramebuffer = [[XDGPUImageContext sharedFramebufferCache] fetchFramebufferForSize:CGSizeMake(bytesPerRow / 4, bufferHeight) onlyTexture:YES];
...
- (void)convertYUVToRGBOutput;
{
    ...
    outputFramebuffer = [[XDGPUImageContext sharedFramebufferCache] fetchFramebufferForSize:CGSizeMake(rotatedImageBufferWidth, rotatedImageBufferHeight) textureOptions:self.outputTextureOptions onlyTexture:NO];
    [outputFramebuffer activateFramebuffer];
	...
	glVertexAttribPointer(yuvConversionTextureCoordinateAttribute, 2, GL_FLOAT, 0, 0, [XDGPUImageFilter textureCoordinatesForRotation:internalRotation]);
...
- (void)setAudioEncodingTarget:(XDGPUImageMovieWriter *)newValue;
{
    if (newValue) {
