...
@interface XDGPUImageMovie () <AVPlayerItemOutputPullDelegate>
{
    BOOL audioEncodingIsFinished, videoEncodingIsFinished;
    XDGPUImageMovieWriter *synchronizedMovieWriter;
...
@implementation XDGPUImageMovie
...
{
    if ([XDGPUImageContext supportsFastTextureUpload])
    {
...
- (void)enableSynchronizedEncodingUsingMovieWriter:(XDGPUImageMovieWriter *)movieWriter;
{
...
- (void)startProcessing
{
    ...
    XDGPUImageMovie __block *blockSelf = self;
...
- (void)processAsset
{
    ...
    __unsafe_unretained XDGPUImageMovie *weakSelf = self;
...
- (void)processPlayerItem
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        dispatch_queue_t videoProcessingQueue = [XDGPUImageContext sharedContextQueue];
...
- (void)processMovieFrame:(CVPixelBufferRef)movieFrame withSampleTime:(CMTime)currentSampleTime
{
    ...
    if ([XDGPUImageContext supportsFastTextureUpload])
    {
        ...
        if (CVPixelBufferGetPlaneCount(movieFrame) > 0) // Check for YUV planar inputs to do RGB conversion
        {
            ...
            {
                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], movieFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE, bufferWidth, bufferHeight, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0, &luminanceTextureRef);
            }
            else
            {
                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], movieFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE, bufferWidth, bufferHeight, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0, &luminanceTextureRef);
            ...
            if ([XDGPUImageContext deviceSupportsRedTextures])
            {
                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], movieFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE_ALPHA, bufferWidth/2, bufferHeight/2, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 1, &chrominanceTextureRef);
            }
            else
            {
                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[XDGPUImageContext sharedImageProcessingContext] coreVideoTextureCache], movieFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE_ALPHA, bufferWidth/2, bufferHeight/2, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 1, &chrominanceTextureRef);
...
//            outputTexture = 0;
    ...
    else
    {
        ...
        outputFramebuffer = [[XDGPUImageContext sharedFramebufferCache] fetchFramebufferForSize:CGSizeMake(bufferWidth, bufferHeight) textureOptions:self.outputTextureOptions onlyTexture:YES];
...
{
    [XDGPUImageContext setActiveShaderProgram:yuvConversionProgram];
    outputFramebuffer = [[XDGPUImageContext sharedFramebufferCache] fetchFramebufferForSize:CGSizeMake(imageBufferWidth, imageBufferHeight) onlyTexture:NO];
