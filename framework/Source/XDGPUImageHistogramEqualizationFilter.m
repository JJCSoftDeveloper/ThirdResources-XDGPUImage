//
//   XDGPUImageHistogramEqualizationFilter.m
//  FilterShowcase
//
...
@implementation  XDGPUImageHistogramEqualizationFilter
...
- (id)initWithHistogramType:(XDGPUImageHistogramType)newHistogramType
{
    ...
    histogramFilter = [[XDGPUImageHistogramFilter alloc] initWithHistogramType:newHistogramType];
    [self addFilter:histogramFilter];
    
    GLubyte dummyInput[4 * 256]; // NB: No way to initialise  XDGPUImageRawDataInput without providing bytes
    rawDataInputFilter = [[XDGPUImageRawDataInput alloc] initWithBytes:dummyInput size:CGSizeMake(256.0, 1.0) pixelFormat:GPUPixelFormatBGRA type:GPUPixelTypeUByte];
    rawDataOutputFilter = [[XDGPUImageRawDataOutput alloc] initWithImageSize:CGSizeMake(256.0, 3.0) resultsInBGRAFormat:YES];
    
    __unsafe_unretained  XDGPUImageRawDataOutput *_rawDataOutputFilter = rawDataOutputFilter;
    __unsafe_unretained  XDGPUImageRawDataInput *_rawDataInputFilter = rawDataInputFilter;
    ...
    switch (newHistogramType) {
     ...
     XDGPUImageFilter *equalizationFilter = [[XDGPUImageTwoInputFilter alloc] initWithFragmentShaderFromString:fragmentShader];
